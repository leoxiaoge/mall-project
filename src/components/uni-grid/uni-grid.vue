	...
	>
		<view v-for="(items,i) in gridGroup" :key="i" class="uni-grid__flex">
			...
			>
				...
				<view class="totals" v-if="isTotals && item.totals > 0">
					<uni-badge :text="item.totals" type="primary" />
				</view>
			</view>
		</view>
	</view>
		...
		gridGroup() {
			...
			if (groupItem.length > 0) {
				if (this.columnNum > groupItem.length) {
					for (
						let i = 0, length = groupItem.length;
						i < this.columnNum - length;
						i++
					) {
						groupItem.push({
							seize: true
...
.uni-grid.uni-grid-no-out-border
	.uni-grid__flex:first-child
	.uni-grid-item:after {
	border-top-width: 0;
}

.uni-grid.uni-grid-no-out-border
	.uni-grid__flex:last-child
	.uni-grid-item:after {
