	...
	<view class="mescroll-uni-warp">
		...
		>
			...
			<view
				...
				class="mescroll-downwarp mescroll-uni-bg"
				:style="{'height': downHight, 'transition': supplyHeight}"
			>
				<view class="downwarp-content">
					<image
						class="downwarp-slogan"
						src="https://api.tengpaisc.com/upload/guidang/icon_slogan.png"
						mode="widthFix"
					/>
					<view
						v-if="!isDownLoading"
						class="downwarp-progress"
						:style="{'transform':downRotate, 'transition': supplyAll}"
					></view>
...
export default {
	...
	computed: {
		...
		// 补帧动画
		supplyHeight() {
			return this.isDownReset
				? "height 300ms"
				: this.isAnimSupply
				? "height " + this.mescroll.optDown.supply + "ms"
				: "";
		},
		supplyAll() {
			return this.isAnimSupply
				? "all " + this.mescroll.optDown.supply + "ms"
				: "";
		}
	},
	methods: {
...
.mescroll-downwarp .downwarp-mascot {
	...
	background-size: contain;
	background-repeat: no-repeat;
	-webkit-animation: animMascot 0.6s steps(1, end) infinite;
	animation: animMascot 0.6s steps(1, end) infinite;
}
